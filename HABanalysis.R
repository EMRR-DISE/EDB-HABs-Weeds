...
sfhaball2 = st_crop(HABssf, reg3)%>%
  st_join(reg3)%>%
  st_drop_geometry() %>%
  filter(!is.na(Stratum2), !is.na(Microcystis)) %>%
...
SFHallzeros = pivot_wider(st_drop_geometry(SFHall2), id_cols = c("Yearm", "Yearf", "Year", "Month", "Stratum"),
...
ggsave("plots/YearHAB_noDOP.tiff", device = "tiff", width = 6, height = 5)
